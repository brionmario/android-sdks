#!/usr/bin/env bash
#
# Run the Android Quickstart E2E suite end to end: start a ThunderID server, provision the test
...
# The Android emulator spawns crashpad_handler (its Google crash-reporter daemon) as a detached
# child that does not receive the signal the emulator process itself gets on shutdown, so it
# outlives it. On reactivecircus/android-emulator-runner in CI, that leftover process is enough
# to deadlock the action's own "Terminate Emulator" step waiting on the process tree - a
# well-documented upstream issue (ReactiveCircus/android-emulator-runner#385) with no fix in the
# action itself. Killing it here, before that step ever runs, is the confirmed community
# workaround. SIGTERM first and only escalate to SIGKILL if it is still alive after a beat -
# harmless on a still-running local emulator, since crashpad is pure crash reporting with no
# effect on the emulator's own operation.
...
build_sample() {
    ...
    # 10.0.2.2 is the emulator's alias for the host loopback, where the server is listening.
    # The debug build sets allowInsecureConnections, which is what lets the app accept the
    # server's self-signed certificate.
    cat > "$sample_dir/config.properties" <<PROPS
THUNDERID_BASE_URL=https://10.0.2.2:8090
THUNDERID_APPLICATION_ID=$APP_ID
THUNDERID_ATTESTATION_ENABLED=false
THUNDERID_CLOUD_PROJECT_NUMBER=
PROPS

    echo "==> Building and installing the sample"
    (cd "$sample_dir" && ./gradlew installDebug)
}
...
run_flows() {
    ...
    (cd "$SCRIPT_DIR" && maestro --platform android test "${target[@]}" \
        --format=JUNIT \
        --output=report.xml \
